...
class PassiveSocialPhobia(BasePassive):
    """
    Социальная фобия.
    Броски кубика на красноречие получают -3.
    """
    id = "social_phobia"
    name = "Социальная фобия"
    description = "Персонажу трудно общаться и имеет мало связей. Броски кубика на красноречие получают -3."
    is_active_ability = False
    ...
    def modify_skill_check_result(self, unit, stat_key: str, current_result: int) -> int:
        ...
        if stat_key == "eloquence":
            logger.log(
                f"💬 {self.name}: {unit.name} получает -3 к проверке красноречия",
                LogLevel.NORMAL, "Passive"
            )
...
class PassiveTopographicCretinism(BasePassive):
    ...
    Броски на ориентирование (мудрость) получают -3.
    """
    id = "topographic_cretinism"
    name = "Топографический кретинизм"
    description = "Броски на ориентирование (мудрость) получают -3."
    is_active_ability = False

    def modify_skill_check_result(self, unit, stat_key: str, current_result: int) -> int:
        """Модифицирует результат проверки ориентирования (мудрости)."""
        if stat_key == "wisdom":
            logger.log(
                f"🧭 {self.name}: {unit.name} получает -3 к проверке ориентирования (мудрости)",
                LogLevel.NORMAL, "Passive"
            )
            return current_result - 3
        return current_result
...
class PassiveSharpMind(BasePassive):
    """
    Хрупкий Разум.
    Персонажу наносится на 20% больше урона от психических атак.
    Броски кубика против необъяснимого получают +4.
    """
    id = "sharp_mind"
    name = "Хрупкий Разум"
    description = "Персонажу наносится на 20% больше урона от психических атак. Броски кубика против необъяснимого получают +4."
    is_active_ability = False

    def modify_skill_check_result(self, unit, stat_key: str, current_result: int) -> int:
        """Модифицирует результат проверки против необъяснимого."""
        if stat_key == "psych":
            logger.log(
                f"🧠 {self.name}: {unit.name} получает +4 к проверке против необъяснимого",
                LogLevel.NORMAL, "Passive"
            )
