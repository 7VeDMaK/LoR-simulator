...
class TalentSafeEGO(BasePassive):
    ...
    name = "Безопасное ЭГО"
    description = (
        "«Ты слышишь Голос, но не поддаешься ему. Ты используешь его силу, оставаясь собой. Это доспех, выкованный из твоей собственной воли.»\n\n"
        "Эффект: Если ваш Рассудок падает до критической отметки, вы гарантированно пробуждаете Э.Г.О., избегая участи стать Искажением."
    )
    is_active_ability = False
...
class TalentControlledDistortion(BasePassive):
    ...
    description = (
        "«Ты позволил Голосу поглотить себя, но в сердце хаоса всё ещё бьется твой разум. Ты — монстр, но поводок всё ещё в твоих руках.»\n\n"
        "Эффект: При ментальном сломе вы становитесь Искажением, но сохраняете контроль над персонажем.\n"
        "Бонус: Ваши характеристики и пассивные способности многократно усиливаются."
    )
    is_active_ability = False