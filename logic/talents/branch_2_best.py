...
class TalentOnTheWaveOfSuccess(BasePassive):
    ...
    is_active_ability = True  # Делаем активным, чтобы можно было лечить SP кнопкой
...
class TalentTacticalAnalysis(BasePassive):
    ...
    cooldown = 2  # Пусть будет небольшой откат

    def activate(self, unit, log_func, opponent=None):
        # В симуляторе мы не всегда знаем оппонента в методе activate,
        # но в render_active_abilities (simulator_components) мы пока не передаем opponent.
        # Для простоты будем считать, что эффект накладывается на "Текущего врага"
        # (Это потребует небольшой доработки в UI, чтобы передать opponent в activate).
        # ПОКА ЧТО сделаем просто бросок и вывод инфо в лог.
        ...
        msg = [f"🎲 **Анализ**: {roll} + {wisdom} (Wis) = **{total}**"]

        # Таблица результатов
        if total <= 3:
            msg.append("1-3: Детальное описание персонажа.")
        elif total <= 6:
            msg.append("4-6: Группа / Ассоциация.")
        elif total <= 8:
            msg.append("7-8: Количество кубов скорости.")
        elif total <= 10:
            msg.append("9-10: Мин/Макс скорость.")
        elif total <= 13:
            msg.append("11-13: Слабые карты (Tier 1).")
        elif total <= 16:
            msg.append("14-16: Средние карты (Tier 2).")
        elif total <= 20:
            msg.append("17-20: Сильные карты (Tier 3).")
        elif total <= 24:
            msg.append("20-24: Мощные карты / EGO.")
        elif total <= 30:
            msg.append("🎯 **25-30: СБОЙ СИСТЕМЫ!**")
            msg.append("Враг получает статус 'Jamming': Откат использованных карт увеличен на +1 (2 хода).")
            # Чтобы это сработало, нам нужно добраться до врага.
            # В текущей архитектуре UI кнопка не знает врага.
            # ВРЕМЕННОЕ РЕШЕНИЕ: Накладываем статус на СЕБЯ с пометкой "Aura",
            # а в clash.py будем проверять ауру врага.
            # ИЛИ (Правильнее): Мы добавим Opponent в аргументы activate чуть позже.
            # Пока просто пишем лог.
        else:
            msg.append("☠️ **36+: УЯЗВИМАЯ ТОЧКА!**")
            msg.append("Враг получает статус 'Vulnerability': Получаемый урон +25%.")
...
class TalentBehaviorStudy(BasePassive):
    ...
    description = (
        ...
        "Пассивно: если ваш рассудок выше 50%, вы знаете примерное положение ваших друзей с погрешностью максимум в 50 метров."
    )
    is_active_ability = False
