...
class TalentTeaMaster(BasePassive):
    ...
    description = (
        ...
        "🫚 **Имбирный чай**: Восстанавливает 15% SP. Вдвое меньше отрицательных эффектов на час.\n"
        "🌺 **Красный чай**: Восстанавливает 15% SP. Дает +1 Силу на час.\n"
        "☕ **Кофе-чай**: Восстанавливает SP... имеет 1% шанс убить вас.\n"
    )
    is_active_ability = True  # Сделаем активным, чтобы можно было открыть меню чая

    def activate(self, unit, log_func, **kwargs):
        if log_func:
            log_func("☕ Меню чаепития открыто (Логика будет позже).")
        return True
