import math
from logic.passives.base_passive import BasePassive
from core.enums import DiceType


# ==========================================
# 3.1 Здоровяк
# ==========================================
class TalentBigGuy(BasePassive):
    id = "big_guy"
    name = "Здоровяк"
    description = "3.1 Увеличивает максимальное здоровье на 15%."
    is_active_ability = False

    def on_calculate_stats(self, unit) -> dict:
        return {"max_hp_pct": 15}


# ==========================================
# 3.2 Оборона (Перенос из zafiel_passives)
# ==========================================
class TalentDefense(BasePassive):
    id = "defense_zafu"  # Сохраняем ID, чтобы работала логика в unit_mixins
    name = "Оборона"
    description = (
        "3.2 Каждый раунд вы получаете кость активного Блока (5-7).\n"
        "3.5: +1 Кость. Победа блоком -> +1 Защита.\n"
        "3.8: +1 Кость. Проигрыш блоком -> +1 Сила.\n"
        "3.10: +1 Кость Контр-блока."
    )
    is_active_ability = False

    # Логика выдачи кубиков находится в core/unit_mixins.py

    # ID технической карты (должен совпадать с unit_mixins)
    card_id_block = "zafu_block_card"

    def on_clash_win(self, ctx):
        # 3.5: Победа -> +1 Защита
        if ctx.source.current_card and ctx.source.current_card.id == self.card_id_block:
            if "talent_3_5" in ctx.source.talents:
                ctx.source.add_status("protection", 1, duration=1)
                ctx.log.append(f"🛡️ **{self.name}**: Победа -> +1 Защита")

    def on_clash_lose(self, ctx):
        # 3.8: Проигрыш -> +1 Сила (желтая стрелка)
        if ctx.source.current_card and ctx.source.current_card.id == self.card_id_block:
            if "talent_3_8" in ctx.source.talents:
                ctx.source.add_status("strength", 1, duration=1)
                ctx.log.append(f"💪 **{self.name}**: Проигрыш -> +1 Сила")


# ==========================================
# 3.3 Похвальное телосложение
# ==========================================
class TalentCommendableConstitution(BasePassive):
    id = "commendable_constitution"
    name = "Похвальное телосложение"
    description = (
        "3.3 Броски Стойкости +2.\n"
        "Пассивно: +1 Стойкость (Endurance). Если есть 3.8 -> +2.\n"
        "Активно (1 раз в день): Короткий отдых. Восстанавливает 20% HP (30%, если есть 3.7)."
    )
    is_active_ability = True
    cooldown = 99  # 1 раз за бой/день

    def on_calculate_stats(self, unit) -> dict:
        # Броски стойкости (навык endurance) +2
        # Пассивный бафф +1 endurance_status реализуем через on_combat_start или тут же?
        # Обычно статусы лучше давать в начале боя. Но тут "Броски стойкости" это навык.
        return {"endurance": 2}

    def on_combat_start(self, unit, log_func, **kwargs):
        # Пассивный бонус к статусу (Щит)
        amt = 1
        if "survivor" in unit.talents:  # 3.8
            amt += 1
        unit.add_status("endurance", amt, duration=99)
        if log_func: log_func(f"🛡️ **{self.name}**: +{amt} Endurance")

    def activate(self, unit, log_func, **kwargs):
        if unit.cooldowns.get(self.id, 0) > 0: return False

        pct = 0.20
        if "tough_as_steel" in unit.talents:  # 3.7
            pct = 0.30

        heal = int(unit.max_hp * pct)
        actual = unit.heal_hp(heal)
        unit.cooldowns[self.id] = self.cooldown

        if log_func: log_func(f"💤 **Отдых**: Восстановлено {actual} HP ({int(pct * 100)}%)")
        return True


# ==========================================
# 3.3 (Опционально) Большое сердце
# ==========================================
class TalentBigHeart(BasePassive):
    id = "big_heart"
    name = "Большое сердце"
    description = (
        "3.3 Опц: Реакцией можно защитить союзника, подставившись под удар (используя неиспользованные кости Блока).\n"
        "Если используются кости Обороны, союзник получает эффекты навыка."
    )
    is_active_ability = False


# ==========================================
# 3.4 Скала
# ==========================================
class TalentRock(BasePassive):
    id = "rock"
    name = "Скала"
    description = (
        "3.4 Если вы не получаете урон при атаке (благодаря резистам/статусам, но не Блоку),\n"
        "весь урон отражается в атакующего."
    )
    is_active_ability = False


# ==========================================
# 3.5 Не взирая на невзгоды
# ==========================================
class TalentDespiteAdversities(BasePassive):
    id = "talent_3_5"  # Используем этот ID для связи с Обороной
    name = "Не взирая на невзгоды"
    description = (
        "3.5 В Оглушении резисты становятся x1.5 (вместо x2.0).\n"
        "Кости навыка 'Оборона' остаются активными даже в Оглушении.\n"
        "Если есть 3.10 -> резисты x1.25."
    )
    is_active_ability = False

    # Логика изменения резистов в Stagger должна быть в calculations.py (пока заглушка)


# ==========================================
# 3.5 (Опционально) Термостойкий
# ==========================================
class TalentHeatResistant(BasePassive):
    id = "heat_resistant"
    name = "Термостойкий"
    description = "3.5 Опц: Урон от Огня и Холода снижен на 33%."
    is_active_ability = False


# ==========================================
# 3.6 Адаптация
# ==========================================
class TalentAdaptationTireless(BasePassive):
    id = "adaptation_tireless"
    name = "Адаптация (Тип 2)"
    description = (
        "3.6 В конце раунда резист к самому частому полученному типу урона снижается на 0.25."
    )
    is_active_ability = False


# ==========================================
# 3.7 Крепкий как сталь
# ==========================================
class TalentToughAsSteel(BasePassive):
    id = "tough_as_steel"
    name = "Крепкий как сталь"
    description = (
        "3.7 Макс. Здоровье +20%.\n"
        "Победа костью блока -> накладывает 1 Хрупкость (Fragile) на врага (макс 3)."
    )
    is_active_ability = False

    def on_calculate_stats(self, unit) -> dict:
        return {"max_hp_pct": 20}

    def on_clash_win(self, ctx):
        if ctx.dice.dtype == DiceType.BLOCK:
            target = ctx.target  # Тот, с кем столкновение (атакующий)
            if target:
                # Ограничение "не больше 3" сложно проверить без истории раунда, 
                # но просто наложим
                target.add_status("fragile", 1, duration=1)
                ctx.log.append(f"🧱 **{self.name}**: Враг получил +1 Хрупкость")


# ==========================================
# 3.7 (Опционально) Защитник
# ==========================================
class TalentDefender(BasePassive):
    id = "defender"
    name = "Защитник"
    description = (
        "3.7 Опц: Союзники получают 4 Защиты в первом раунде.\n"
        "Можно перехватывать удары за союзников без костей блока (получая +1 Силу за каждый удар)."
    )
    is_active_ability = False

    def on_combat_start(self, unit, log_func, **kwargs):
        # В текущей версии сложно найти "союзников", наложим на себя как ауру
        if log_func: log_func(f"🛡️ **{self.name}**: Аура защиты активирована.")


# ==========================================
# 3.8 Выживший
# ==========================================
class TalentSurvivor(BasePassive):
    id = "talent_3_8"  # Связь с Обороной
    name = "Выживший"
    description = (
        "3.8 Броски Стойкости с преимуществом.\n"
        "Внезапная атака наносит x1.5 урона (вместо x2.0).\n"
        "Урон от Кровотечения снижен на 33%.\n"
        "Активно: Если в раунде не использовали кости -> Восстановить 15% HP."
    )
    is_active_ability = True
    cooldown = 99

    def activate(self, unit, log_func, **kwargs):
        # Проверка "не использовали кости" (сложно, считаем что игрок честный)
        if unit.cooldowns.get(self.id, 0) > 0: return False

        heal = int(unit.max_hp * 0.15)
        unit.heal_hp(heal)
        unit.cooldowns[self.id] = self.cooldown
        if log_func: log_func(f"❤️ **Выживший**: Восстановлено {heal} HP")
        return True


# ==========================================
# 3.9 Перенапряжение мышц
# ==========================================
class TalentMuscleOverstrain(BasePassive):
    id = "muscle_overstrain"
    name = "Перенапряжение мышц"
    description = "3.9 Активно: Потратить 5 HP или 10 Stagger -> +1 Мощь кубиков (2 раза/раунд)."
    is_active_ability = True

    def activate(self, unit, log_func, **kwargs):
        # Тратим 5 HP
        unit.current_hp = max(1, unit.current_hp - 5)
        unit.add_status("strength", 1, duration=1)
        if log_func: log_func("💪 **Перенапряжение**: -5 HP -> +1 Сила")
        return True


# ==========================================
# 3.9 (Опционально) Клятва идола
# ==========================================
class TalentIdolOath(BasePassive):
    id = "idol_oath"
    name = "Клятва идола"
    description = (
        "3.9 Опц: Вы отказываетесь от лечения других.\n"
        "Медицина +15.\n"
        "HP < 25% -> +2 Мощь.\n"
        "Крепкая кожа +15."
    )
    is_active_ability = False

    def on_calculate_stats(self, unit) -> dict:
        mods = {"medicine": 15, "tough_skin": 15}
        # Проверка HP < 25%
        if unit.max_hp > 0 and (unit.current_hp / unit.max_hp) < 0.25:
            mods["power_all"] = 2  # Гипотетический модификатор ко всем броскам
        return mods


# ==========================================
# 3.10 Прилив сил
# ==========================================
class TalentSurgeOfStrength(BasePassive):
    id = "talent_3_10"  # Связь с Обороной
    name = "Прилив сил"
    description = (
        "3.10 HP < 25% -> Мгновенный выход из Оглушения и переброс инициативы.\n"
        "До конца раунда: +4 Силы, Стойкости, Спешки, Защиты.\n"
        "Далее до конца боя: +2 Спешки, Откаты -1."
    )
    is_active_ability = False

    # Логика "HP < 25%" должна проверяться в on_take_damage или on_round_start