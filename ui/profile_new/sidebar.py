...
from ui.profile_new.sidebar_parts.resistances import render_resistances
...
def render_sidebar(unit, is_edit_mode: bool):
    ...
    # 3. Показатели (HP/SP/Stagger) - ТЕПЕРЬ С РЕДАКТИРОВАНИЕМ
    render_vital_stats(unit, is_edit_mode)

    st.divider()

    # 4. Резисты
    render_resistances(unit, is_edit_mode)